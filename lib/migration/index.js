...
class Migration {
    ...
    reset(options = {}) {
        ...
        if (!this.migrationDirectory.configMigration.sgbd) {

            return Promise.reject(new Error('Migração não foi configurada. Execute: migration config'));
    ...
    /**
     * Instala Modelos de outros modulos instalados via NPM
     */
    install() {


        return this.migrationDirectory.findModels()
            .then(models => {

                let modules = Object.keys(models);
                ...
                return Promise
                    .map(modules, moduleName => {
                        ...
                        let destinyPath = path.join(this.migrationDirectory.dirModels, moduleName);

                        return this.migrationDirectory.exist(path.join(destinyPath)).then(exist => {

                            return {
                                name: moduleName + (exist ? chalk.bold(' (INSTALADO)') : ''),
                                checked: !exist,
                                disabled: exist
                            };
                    ...
                    .then(choices => this.ui.checkbox('Selecione os modulos que deseja instalar:', choices))
                    ...
                    .then(result => {
                        ...
                        if (result.length > 0) {
                            ...
                            logger.info();
                            logger.info('Os seguintes modelos serão instalados:');

                            for (let module of result) {
                                logger.info(chalk.green(`---- ${module} ----`));

                                for (let model of models[module]) {
                                    logger.info(`- ${path.basename(model)}`);
                                }
                            }
                            logger.info();

                            return this.ui.confirm('Confirmar?', false);
                        } else {

                            logger.info(chalk.bold.green('Nenhum módulo para instalar.'));
                            return;
                        }
                    ...
                    .then(confirm => {
                        ...
                        if (confirm) {
                            ...
                            return Promise.mapSeries(modules, module => {

                                return this.migrationDirectory.copyModels(module, models[module]);
