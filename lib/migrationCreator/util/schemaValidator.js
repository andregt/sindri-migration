 ...
 * **Created on 14/12/16**
 *
 * sindri-migration/lib/migrationCreator/util/schemaValidator.js
 * @author André Timermann <andre@andregustvo.org>
 *
 * TODO: Validar aspas nos comentários
 */
'use strict';
...
class SchemaValidator {
    ...
    _validatePrimaryKey(tableName, tableInfo) {
        ...
        if (!tableInfo.primaryKey || (tableInfo.primaryKey && tableInfo.primaryKey.length === 0)) {
            this.validateResult.error.push(`Na tabela '${tableName.bold}' a chave primária não foi definida!`);
        }


        if (tableInfo.primaryKey && tableInfo.primaryKey.length > 1) {
            this.validateResult.error.push(`Na tabela '${tableName.bold}' deve existir apenas uma chave primária!`);
        }
        ...
        for (let [columnName, column] of Object.entries(tableInfo.columns)) {
            ...
            if (!column.type) {
                ...
                if (!_.includes(validKnexTypes, typeLower)) {
                    ...
                    this.validateResult.warn.push(`Na Coluna '${columnName.bold}' da tabela '${tableName.bold}', você está usando um tipo não padrão: '${typeLower.bold}'. Para manter compatibilidade entre diversos SGBDs na hora de gerar a base de dados, use um dos seguintes tipos: '${validKnexTypes.join(', ').bold}'. Verifique a documentação do KnexJs!\n`);
