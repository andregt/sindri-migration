...
class SchemaValidation {
    ...
    static _validateColumns(tableInfo, tableName, validateResult) {
        ...
        if (!tableInfo.columns) {
            validateResult.error.push(`Colunas não defindas na tabela '${tableName.bold}'. Verifique o atributo ${'columns'.bold}!`);
            return;
        }
        ...
        if (tableInfo.columns.length == 0) {
            validateResult.error.push(`Não foi definido colunas na tabela '${tableName.bold}'`);
